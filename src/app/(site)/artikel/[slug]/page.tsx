...
async function getPopularTags(currentSlug: string) {
  const articles = await getPublicArticles();
...
export default async function ArticleDetailPage({
  ...
  const popularTags = [...new Set([...article.tags, ...(await getPopularTags(article.slug))])].slice(0, 6);
      ...
      <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:gap-14">
        ...
        <aside className="space-y-8 lg:col-span-4 lg:sticky lg:top-28 lg:self-start">
          ...
          <section className="rounded-[1.75rem] bg-[var(--secondary-container)] p-6">
            <h3 className="text-lg font-bold text-[var(--on-secondary-fixed)]">Ringkasan Artikel</h3>
